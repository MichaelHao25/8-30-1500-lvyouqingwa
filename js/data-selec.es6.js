...
(function () {
	...
	function renderDomElements(yaer, month) {
		...
		renderDays(querySelectorAll('.date-select-container .day-container')[0], yaer, month);
		addSelect();
	}
	//渲染骨架
	...
	function bindEvent() {
		...
		addEventListener({
			...
			event: 'click',
			function: function (e) {
				var taggetElement = e.path.find((value, index) => {
	...
	function init() {
		...
		setAttr(querySelectorAll('.date-select-container')[0], 'currentDate', initY + '-' + initM + '-' + initD);
